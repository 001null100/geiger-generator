...
public:
    ...
    double balanceGain() const noexcept { return balance_; }
...
private:
    struct Channel {
        ...
        double outIn=0, outDc=0, driveIn=0, safetyIn=0;
    };
    void updateCoefficients() noexcept;
    void reseed() noexcept;
    double nextHazard() noexcept;
    bool detect(double time, bool secondary) noexcept;
    void excite(double amplitude, double fraction=0) noexcept;
    double render(Channel& ch, double injection, double variation, double noiseEnergy, double noise, double hum) noexcept;
    Values values_=defaults();
    Random timing_, color_, field_, noise_, stereoNoise_;
    std::array<Channel,2> channels_{};
    std::array<double,16> afterTimes_{};
    std::array<double,2> pending_{}, modalPending_{}, energyPending_{};
    std::array<double,2> nextPending_{}, nextModal_{}, nextEnergy_{};
    ...
    double pulseMix_=1, ringMix_=1, noiseMix_=0.1, gainTarget_=0.25, drive_=1, driveTarget_=1, balance_=1, balanceA_=0;
